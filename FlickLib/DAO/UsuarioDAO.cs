...
{
    class UsuarioDAO : IDataAccess<Usuario>
    {
        ...
        internal Cliente inicioSesion(Usuario Entidad)
        {
            using (SqlCommand cmd = new SqlCommand(null, Configuraciones.connect))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                {
                    if (reader.Read()) return new Cliente(reader);
                    else return null;
                }
            }
        }
        ...
        internal int ContarRepetidos(string usuario_username)
        {
            using (SqlCommand cmd = new SqlCommand(null, Configuraciones.connect))
            {
